...
export async function getCurrentUser() {
  try {
    ...
    // Silent session auto-recovery fallback: attempt token refresh if expired
    try {
      ...
      // Ignore refresh error and return null
    }
...
export async function requireCallAccess(callId) {
  const { user, profile } = await requireAuth();
  const admin = createAdminClient();
  const { data: session } = await admin
    .from("call_sessions")
    .select("id, hostel_id, parent:parents(user_id), student:students(user_id)")
    .eq("id", callId)
    .maybeSingle();

  if (!session) throw new AuthError("Call session not found", 404, "CALL_NOT_FOUND");
  if (profile.role === "SUPER_ADMIN") return { user, profile, session };
  if (["HOSTEL_ADMIN", "WARDEN", "STAFF"].includes(profile.role)) {
    await requireHostelAccess(session.hostel_id, ["HOSTEL_ADMIN", "WARDEN", "STAFF"]);
    return { user, profile, session };
  }
  if (session.parent?.user_id === user.id || session.student?.user_id === user.id) {
    return { user, profile, session };
  }
  throw new AuthError("You do not have permission to access this call", 403, "FORBIDDEN");
}
