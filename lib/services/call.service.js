...
export async function createCallSession({
  ...
  // 3. Quota check: max calls per student per day (skip for emergency)
  if (!isEmergency) {
    ...
    if (studentDailyCalls >= (hostel.max_calls_per_student_per_day || 2)) {
      throw new CallServiceError(
        `Daily call limit reached for this student (max ${hostel.max_calls_per_student_per_day || 2} calls/day)`,
